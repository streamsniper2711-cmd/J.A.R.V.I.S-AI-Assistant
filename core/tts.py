...
def create_tts_player(config: dict) -> TTSPlayer:
    ...
    if engine_name == "kokoro":
        ...
        engine = ElevenLabsTTSEngine(
            ...
            voice_id=config.get("elevenlabs_voice_id") or config.get("tts_voice", ELEVENLABS_DEFAULT_VOICE),
            model_id=config.get("elevenlabs_model_id", ELEVENLABS_DEFAULT_MODEL),
        )
    elif engine_name in {"fish_audio", "fish", "fishaudio"}:
        engine = FishAudioTTSEngine(
            api_key=config.get("fish_audio_api_key", ""),
            model_id=config.get("fish_audio_model_id", "s2-pro"),
            voice_id=config.get("fish_audio_voice_id", ""),
            endpoint=config.get("fish_audio_endpoint", "https://api.fish.audio/v1/tts"),
            audio_format=config.get("fish_audio_format", "mp3"),
            latency=config.get("fish_audio_latency", "normal"),
        )
    else:   # edgetts (default)
        voice = config.get("tts_voice", "en-GB-RyanNeural")
        speed = float(config.get("voice_speed", config.get("tts_speed", 1.0)))
        pitch = float(config.get("voice_pitch", 0.0))
        rate = f"{round((speed - 1.0) * 100):+d}%"
        pitch_hz = f"{round(pitch * 12):+d}Hz"
        engine = EdgeTTSEngine(voice=voice, rate=rate, pitch=pitch_hz)
