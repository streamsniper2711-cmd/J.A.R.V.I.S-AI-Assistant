...
def _handle_play(parameters: dict, player) -> str:
    ...
    if video_url:
        ...
        if player is not None and hasattr(player, "show_video"):
            player.show_video(video_url)  # plays inline inside the JARVIS UI
        else:
            _open_url(video_url)          # fallback: external browser
